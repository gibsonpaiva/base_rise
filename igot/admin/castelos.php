...
<?php
    ...
    $postURL = "'ajax/updCastelos.php'"; // Endereço da página com as funções SQL desta Base de Dados / Tabela
    $tr_table = "";
    for ($i=0; $i<count($itens); $i++){
        $tr_table .= '
            <tr id="'.$itens[$i]['id'].'" name="'.$itens[$i]['id'].'">
                ...
                <td class="datagrid">
                    <span class="viewOnly Castelo">'.$itens[$i]['Nome'].'</span>
                    <input class="editInput form-control input-sm" type="text" name="Castelo" value="'.$itens[$i]['Nome'].'" style="display:none;">
                </td>
                <td class="datagrid actions" align="center">
                    <button onClick="EditCastelo(this.id)" id="btnEdit'.$itens[$i]['id'].'" type="button" class="btn btn-sm btn-default btnEdit" style="float:none;"><span class="glyphicon glyphicon-pencil"></span></button>
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                ...
                                <div class="box-header">
                                    <h3 class="box-title">Castelos</h3>
                                </div>
                                <!-- Conteúdo da Caixa -->
                                <div class="box-body">
                                    <!-- Botão Novo -->
        ...
        <script>
            ...
            function New(){
                ...
                '<tr class="ui-sortable-handle" id="NewRecord">'
                    ...
                    +'<td class="datagrid">'
                        ...
                        +'<input class="editInput form-control input-sm" type="text" name="Castelo">'
                    +'</td>'
                    +'<td class="datagrid actions" align="center">'
                        +'<button onClick="EditCastelo(this.id)" id="btnEdit" type="button" class="btn btn-sm btn-default btnEdit" style="float:none; display:none;"><span class="glyphicon glyphicon-pencil"></span></button>'
            ...
            function SaveNew(){
                ...
                $.ajax({
                    ...
                    success:function(response){
                        ...
                        trObj.find(".viewAlways.id").text(response.data.idCastelo);
            ...
            function Update(btnID){
                ...
                $.ajax({
                    ...
                    type:'POST',
                    dataType: "json",
                    data:'action=update&idCastelo='+ID+'&'+inputData,
                    success:function(response){
                        // Atualiza o texto dos campos de visualização
                        trObj.find(".viewOnly.Reino").text(response.data.Reino);
                        trObj.find(".viewOnly.Castelo").text(response.data.Castelo);
