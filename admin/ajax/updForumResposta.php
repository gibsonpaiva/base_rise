<?php
    ...
    switch($_POST['action']){
        case 'insert':
            // SQL Query
            $sql = "INSERT INTO ".TBL_FORUMRESPOSTAS." (idTopico, Resposta, RegistradoPor) VALUES ({$_POST['idTopico']}, '{$_POST['Resposta']}', {$_SESSION['user']['id']})";
            // Executa a Query
            $db->query($sql);
        
            // SQL Query
            $sql = 'SELECT idTopico, idResposta, Resposta, RegistradoEm, idRegistradoPor FROM ".VIEW_FORUMRESPOSTAS." WHERE idTopico='.$_POST['idTopico'].' AND idRegistradoPor='.$_SESSION['user']['id'].' ORDER BY idResposta DESC LIMIT 1';
            // Executa a Query
            $result = $db->query($sql);
        ...
        case 'update':
            $sql = "UPDATE ".TBL_FORUMRESPOSTAS." SET Resposta='{$_POST['FormEditResposta']}', ModificadoPor={$_SESSION['user']['id']} WHERE idResposta = '{$_POST['idResposta']}'";
            // Executa a Query
            $db->query($sql);
            // SQL Query
        ...
        case 'delete':
            // SQL Query
            $sql = "DELETE FROM ".TBL_FORUMRESPOSTAS." WHERE idResposta={$_POST['idResposta']}";
            // Executa a Query
            $db->query($sql);
